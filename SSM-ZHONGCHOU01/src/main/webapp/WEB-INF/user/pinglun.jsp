...
<body>
	<div id="header">
		<div class="heade-con">
			...
			<div class="index-login">
				<a href="login.html">登录</a> <a href="user.html">注册</a>
			</div>
		</div>
	</div>
	<script>
	...
	<div class="user-con">
			...
			<div class="user-conr_tab2">
				<div id="d1">
					<table>
						...
						<c:forEach items="${pinglist}" var="ping" varStatus="a">
						<tr>
							<td>${a.count}</td>
							<td>${ping.aim }</td>
							<td>${ping.money }</td>
							<td>${ping.day }</td>
							<td><img alt="" src="upload/project/${ping.fileid}"></td>
							<td><button onclick="tianjia(${ping.id})">进行评论</button>
							</td>
		                 </tr>
      </c:forEach>
					</table>
					...
					<!-- 将评论的数据进行得到总的数据 所有评论的查看-->
					<c:forEach items="${pinglunlist}" var="pinglun" varStatus="a">
					<tr>
					<td>${pinglun.user.name}</td>
					<td>${pinglun.project.money}</td>
					<td><img alt="" src="upload/project/${pinglun.project.fileid}"></td>
					<td>${pinglun.project.aim}</td>
      ...
      <div id="d2">
				<form action="addpinglun">
					<!-- 项目的名称 -->
					<input type="text" class="form-control" 
						 id="proid">
					<!-- 评论内容 -->
					<textarea class="form-control" rows="3" name="ping"></textarea>
